...
/**
 * The Tapstate CLI: the surface-ring product front-end. Bare {@code tapstate} opens the terminal
 * workbench; one-shot subcommands share the same verb table for scripting / AI.
 *
...
@Command(name = "tapstate", mixinStandardHelpOptions = true, version = Cli.VERSION,
        ...
        description = {
                "",
                "With no command, opens the full-screen terminal workbench. The command bar and",
                "command palette are the only interactive surfaces. Session commands are listed below.",
                "",
...
public final class Cli implements Runnable {
    ...
    public static CommandLine newCommandLine() {
        return CommandRegistry.standard(io.tapstate.core.schema.SchemaNavigator.bundled()).commandLine();
    }
