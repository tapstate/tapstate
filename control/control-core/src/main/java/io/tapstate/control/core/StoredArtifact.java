 ...
 * and the content hash of the authoritative resource. A Source presentation omits connector config;
 * applying it back to the same stored Source retains that config. Its bytes do not determine the hash. The server,
 ...
 * authoritative resource, so the two never need reconciling.
 */
