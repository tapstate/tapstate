...
class RealMysqlToMongoSharedPositionIT {
    ...
    void aSharedPhysicalReaderExpandsItsTablesAndBothPipelinesResumeAfterRestart(Tiers tier) throws Exception {
        ...
        try (MongoEndpoints mongo = new MongoEndpoints()) {
            try (ServerHandle server = tier.launch(store)) {
                ...
                Await.until("the first reader's confirmed tail position",
                ...
                assertPhysicalUnion(store);
                assertThat(identityIfPresent(store, second, "second_source"))
                        .as("the attached pipeline has no independent connector stream identity")
                        .isNull();
            ...
            try (ServerHandle server = tier.launch(store)) {
                ...
                assertThat(identity(store, first, "first_source")).isEqualTo(firstIdentity);
                byte[] laterOwner = identityIfPresent(store, second, "second_source");
                if (laterOwner != null) {
                    assertThat(laterOwner).as("a later owner keeps its own connector identity")
    ...
    private static byte[] identity(String store, String pipeline, String source) {
        byte[] identity = identityIfPresent(store, pipeline, source);
        assertThat(identity).as("the reader identity is durable for %s/%s", pipeline, source).isNotNull();
        return identity;
    }

    private static byte[] identityIfPresent(String store, String pipeline, String source) {
    ...
    private static void assertPhysicalUnion(String store) {
        String database = new ConnectionString(store).getDatabase();
        assertThat(database).isNotBlank();
        try (MongoClient client = MongoClients.create(store)) {
            Document union = client.getDatabase(database).getCollection("srs_meta")
                    .find(new Document("physicalCaptureTables",
                            new Document("$all", List.of("orders", "later_orders"))))
                    .first();
            assertThat(union).as("one durable physical subscription includes both selected tables").isNotNull();
            assertThat(union.getLong("physicalCaptureRevision"))
                    .as("the table union was published after replacing the first subscription")
