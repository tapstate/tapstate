...
enum ActuationError implements TapstateErrorCode {
    ...
    /**
     * A view's declared key is not the identity of an assembled document that feeds it; {@code view}
     * is its id, {@code key} the view's key, {@code identity} the nest's explicitly declared root key.
     ...
     * single snapshot. A discovered table key is only a default and does not override an explicitly
     * declared view key.
     */
